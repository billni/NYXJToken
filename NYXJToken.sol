...
contract NYXJToken is StandardToken, SafeMath {
    ...
    function setTokenExchangeRate(uint256 _tokenExchangeRate) isOwner external {
        ...
        if (_tokenExchangeRate == 0) return ;

        if (_tokenExchangeRate == tokenExchangeRate) return;
    ...
    function increaseSupply (uint256 _value) isOwner external {
        ...
        if (value + currentSupply > totalSupply) return;
    ...
    function setMigrateContract(address _newContractAddr) isOwner external {
        ...
        if (_newContractAddr == newContractAddr) return;
    ...
    function changeOwner(address _newFundDeposit) isOwner() external {
        ...
        if (_newFundDeposit == address(0x0)) return;
    ...
    function migrate() external {
        ...
        if(newContractAddr == address(0x0)) return;
        ...
        if (!newContract.migrate(msg.sender, tokens)) return;
    ...
    function transferETH() isOwner external {
        ...
        if (!ethFundDeposit.send(balanceOf(msg.sender))) return;
    ...
    function () payable public {
        ...
        if (block.number > fundingStopBlock) return;
